...
- **Firefox / Thunderbird**: the same four families, applied to browser/mail
  chrome (toolbar, tabs, urlbar, folder pane, menus) via `userChrome.css`/
  `userContent.css` rather than a WebExtension theme, so it works on a
  manually-installed, unpackaged build without enabling unsigned
  extensions. See [`firefox/README.md`](firefox/README.md) and
  [`thunderbird/README.md`](thunderbird/README.md). Generated by
  `bin/gen-browser-themes.py` from the same palettes as everything else;
  no separate switcher, the four `<family>-theme` scripts below also
  cover these two (restart either app to see a switch take effect).
